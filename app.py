...
with st.expander("🧠 Buka untuk melihat proses berpikir & asal rumus hitungan di atas"):
    st.markdown("""
    ...
    * **Biaya Tambahan per KL** = 6,2898 × Kurs
    * **Beban Subsidi** = Volume (KL) × Biaya Tambahan per KL
    * **Pendapatan Bruto Setahun** = Lifting × 365 × Kurs
    * **Defisit per $1** = Total Beban - (PNBP SDA + PPh Migas)
    """)

# --- VISUALISASI GRAFIK BARU ---
st.divider()
...
with col_chart1:
    ...
    # Dibagi 1 juta supaya angkanya enak dilihat di grafik
    df_vol = pd.DataFrame({
...
with col_chart2:
    ...
    df_fin = pd.DataFrame({
        "Komponen": ["Beban Pertalite", "Beban Solar", "Beban Mitan", "PNBP SDA", "PPh Migas"],
        "Nilai (Triliun Rp)": [beban_pertalite, beban_solar, beban_mitan, pnbp_sda, pph_migas]
    })
    st.bar_chart(df_fin.set_index("Komponen"))
